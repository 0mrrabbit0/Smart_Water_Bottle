...
/* Battery icon at top-right: 14x8 frame + 2x4 positive terminal,
 * inner fill proportional to capacity, percentage text below. */
static void DrawBatteryIcon(uint8_t capacity)
{
    ...
    for (uint8_t i = 0; i < width; i++) {
        BSP_OLED_DrawPoint(x + i, y, 1);
        BSP_OLED_DrawPoint(x + i, y + height - 1, 1);
    }
    for (uint8_t i = 0; i < height; i++) {
        BSP_OLED_DrawPoint(x, y + i, 1);
        BSP_OLED_DrawPoint(x + width - 1, y + i, 1);
    }
    ...
    uint8_t fill_width = ((width - 4) * capacity) / 100;
